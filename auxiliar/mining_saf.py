...
class SAFAnalyzer:
    """Classe para análise de Sistemas de Acompanhamento de Falhas (SAFs)"""
    ...
    def filtrar_linha(self, linha: str) -> pd.DataFrame:
        ...
        if veiculos:
            filtro = filtro & self.df['Veículo'].isin(veiculos)
    ...
    def _get_mes_ano_referencia(self, df_linha: pd.DataFrame) -> Tuple[int, int]:
        ...
        primeira_data = df_linha['Data de Abertura'].iloc[0]
        return primeira_data.month, primeira_data.year
    
    def processar_linha(self, linha: str) -> None:
        """Processa e conta as SAFs para uma linha específica"""
        ...
        for dia in range(1, 32):
            try:
                # Filtrar por data
                mask_data = (df_linha['Data de Abertura'].dt.day == dia) & \
                           (df_linha['Data de Abertura'].dt.month == mes) & \
                           (df_linha['Data de Abertura'].dt.year == ano)
    ...
    def salvar_na_planilha_existente(self, template_path: str, output_path: str) -> None:
        """Preenche a planilha template com os dados processados"""
        try:
...
def main():
    ...
    try:
        ...
        downloads_path = pathlib.Path.home() / "Downloads" / "Documentos - Farol" / "SafCompleta.csv"
        ...
        analyzer = SAFAnalyzer(str(downloads_path))
